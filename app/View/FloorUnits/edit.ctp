...
<div id="unidad1">
    <fieldset>
        <?php echo $this->Form->create("FloorUnit", array('id' => 'formulario', "action" => "edit/" . $this->data['FloorUnit']['id'])); ?>
        <legend>Edición de Unidades de Suelo</legend>
        <?php echo $this->Form->hidden('FloorUnit.id') ?>   
        <?php echo $this->Form->hidden('FloorUnit.sincronizado',array('value'=>0)) ?>   
        <?php echo $this->Form->hidden('FloorUnit.property_id', array('type' => 'text')); ?>
