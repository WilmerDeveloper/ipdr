...
<div id="hidricos">
    <fieldset>
        <?php echo $this->Form->create("WaterResource", array('id' => 'formulario', "action" => "edit/" . $this->data['WaterResource']['id'])); ?>
        <legend>Edición de de Recursos Hídricos</legend>
        <!--        Otra vez el error es poner el value,  no debe esara ni "value=>''" -->
        <?php echo $this->Form->hidden('WaterResource.property_id', array('type' => 'text')); ?>
        <?php echo $this->Form->hidden('WaterResource.sincronizado', array('value' => 0)); ?>
        <?php echo $this->Form->hidden('WaterResource.id', array('type' => 'text')); ?>
        ...
        <fieldset><legend>Tipos de uso del agua</legend>
            ...
            <?php echo $this->Form->input('WaterResource.u
                so_agua_agricultura', array('label' => 'Uso de agua Agricultura', 'class' => '', 'type' => 'checkbox')); ?>
